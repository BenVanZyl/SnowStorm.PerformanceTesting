...
namespace PerformanceTesting.Server.Services.Data
{
    public class Order: IDomainEntity
    {
        ...
        internal static async Task<Order> Create(IQueryExecutor executor, OrderDto data, bool autoSave = true)
        {
            if (data == null)
                throw new Exception("Create Failed due to missing data!: Orders");

            var v = new Order(data);
            await executor.Add<Order>(v);
